...
// pictures
import logoCea from "../../../../public/images/logo_final_cea_universal_sin_fondo.png"
import logoCeafondo from "../../../../public/images/logoceasinbg.png"
...
import { ModalPrincipal } from '../../components/Modal/Modals';
import { PermisosGlobalStore } from '../../../store/PermisosGlobalStore';
import Input from '../../components/input/Input';
import { useRegister } from '../../../api/Controller/Seguridad/auth/registerController';
...
export interface ICambiarContrasenia {
	contraseniaActual: string;
	nuevaContrasenia: string;
}
const cambiarContraseniaDefault: ICambiarContrasenia = {
	contraseniaActual: '',
	nuevaContrasenia: ''
}
...
const Header = () => {
	...
	const [cambiarContrasenia, setCambiarContrasenia] = useState<ICambiarContrasenia>(cambiarContraseniaDefault);
	const [ModalcambiarContrasenia, setModalCambiarContrasenia] = useState(false);
	...
	const handleSubmit = (event) => {
		...
		if (form.checkValidity() === false) {
			...
			event.preventDefault();
			setValidated(false)
			renderCambiarContrasenia();
		};
	};

	const renderCambiarContrasenia = async () => {
		setDisabled(true)
		try {
			const response = await Update.ChangePassword(cambiarContrasenia)
			alertglobal("Exito", response.message, 'success')
			setModalCambiarContrasenia(false)
		} catch (error: any) {
			alertglobal("Info", error.response.data.message, 'warning')
		} finally {
			setDisabled(false)
		}
	}
	...
	return (
		<div className={styles.Header}>
					...
					<div className="main-header-left ">
						<div className="responsive-logo">
							<div className="header-logo">
								<img src={logoCea} style={{ width: '46px', height: 'auto' }} className="logo-1" alt="logo" />
								<img src={logoCeafondo} className="dark-logo-1" alt="logo" style={{ width: '46px', height: 'auto' }} />
							</div>
							...
							<div className="card search-result p-absolute w-40 search-fix  border mt-1">
								...
								<ListGroup className='mt-2'>
									{show2 ?
										...
										: <b className={`${searchcolor} text-center my-2`}>{searchval}</b>}
								</ListGroup>
							...
							<Navbar.Collapse className="collapse navbar-collapse" id="navbarSupportedContent-4">
								<Nav className="nav-item navbar-nav-right ms-auto">
									...
									{/* <Dropdown className="nav-item flag_content countries">
										<Dropdown.Toggle href="#" variant='' className="d-flex nav-link nav-item country-flag1">
									...
									<Dropdown.Toggle className="new nav-link" variant="">
										{permisos.PermisosGlobal.isGlobal && (
											<span style={{ cursor: 'pointer' }}
												onClick={() => {
													setModalCambiarContrasenia(!ModalcambiarContrasenia);
													const cambiarContraseniaDefault: ICambiarContrasenia = {
														contraseniaActual: '',
														nuevaContrasenia: ''
													}
													setCambiarContrasenia(cambiarContraseniaDefault)
													setValidated(false)
												}}
										...
										<Dropdown.Menu style={{ margin: '0px' }} >
											...
											<PerfectScrollbar>
												...
												<div className="main-message-list chat-scroll position-absolute">
													...
													<Dropdown.Item href={`${import.meta.env.BASE_URL}pages/mail/chat`} className="p-3 d-flex border-bottom text-wrap">
			...
			{permisos.PermisosGlobal.isGlobal && (
				<ModalPrincipal
					open={ModalcambiarContrasenia}
					setOpen={() => setModalCambiarContrasenia(!ModalcambiarContrasenia)}
					children={
						<>
							<Form className="form-horizontal" noValidate validated={validate} onSubmit={handleSubmit}>
								<Input label="Contraseña Actual" className="mb-3" type="text" placeholder="Contraseña Actual"
									value={cambiarContrasenia.contraseniaActual}
									disabled={disabled}
									required
									onChange={({ target: { value } }: React.ChangeEvent<HTMLInputElement>) => setCambiarContrasenia((prev) => ({ ...prev, contraseniaActual: value }))}
								/>
								<Input label="Nueva Contraseña" className="mb-3" type="text" placeholder="Nueva Contraseña"
									value={cambiarContrasenia.nuevaContrasenia}
									disabled={disabled}
									required
									onChange={({ target: { value } }: React.ChangeEvent<HTMLInputElement>) => setCambiarContrasenia((prev) => ({ ...prev, nuevaContrasenia: value }))}
								/>
								<div>
